...
- **Title screen** (boots to PIRATES FOLLY + a lone ship on the horizon; any
  A/START -> seed editor).
...
- **Port markers**: docking at a port inks a house glyph into that cell on the
  chart (START). Persisted in the save (v3).
- **Ports render in-world**: beaches in port districts get plank-dock tiles —
  you can spot a port from the sea. Pure overlay on the hash, so the
  same-seed-same-world guarantee holds (and a dock tile exists iff you can dock).
- **Difficulty tuning** (`tests/tune_balance.py`): automated duels with a
  stationary auto-firing player set the balance floor — pirate: sunk in ~1.4 s
  costing 1 hull; guardian: ~2.4 s, 2 hull. Port access measured: nearest
  dockable port 2 districts from spawn; 100% of dockable ports have another
  within tavern range. Combat knobs live in defs.inc (`PIRATE_HP`,
  `PIRATE_FIRECOOL`, ...); fire rates bumped ~20% (75/50) since the floor was
  too gentle.
...
- **Enemy fire control read the velocity byte as a range** — pirates
  approaching westward never fired while closing, and long-range ones fired
  while approaching eastward. Now tracks true Chebyshev range (`wEvC`).
- **Random encounters could camp isle cells and block guardians** — a pirate
  rolled on the newly-charted isle cell prevented its guardian (win-condition
  critical) from spawning. Isle cells no longer roll random encounters.
...
ROM budget (v1.0): ROM0 14068/16384 B used, WRAM0 518/8192 B, HRAM 10/127 B.
